...
uses
  Classes, SysUtils, FileUtil, TATransformations, Forms,
...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure wwodxmChange(Sender: TObject);
    procedure wwodxmKeyPress(Sender: TObject; var Key: char);
...
begin
 ...
 if unit1.form1.wwodxm.text='' then exit;
 if unit1.form1.wwody.text='' then exit;

 //Инициализация массивов с введенными данными

 nx:=0;
 for i:=0 to unit1.form1.wwodxm.Lines.Count-1 do
  if unit1.form1.wwodxm.Lines[i]<>'' then begin
  inc(nx); setlength(xmas,nx);
  xmas[nx-1]:=strtofloat(unit1.form1.wwodxm.Lines[i]);
  end;
 ...
 if nx<3 then begin
 showmessage('Количество точек должно быть больше 2');
 exit;
 end;
 ...
 unit4.graf.Width:=770;
 unit4.graf.Height:=unit1.form1.Height-5;
 ...
 unit4.graf.top:=unit1.form1.Top;
...
begin
  ...
  if statobr.Checked then begin
   ...
   combobox1.left:=110;
   ochisti.Caption:='Очистить поля ввода';

   unit4.graf.Left:=form1.Left+255;
   unit4.graf.Top:=form1.top;
   unit4.graf.Width:=770;
   unit4.graf.Height:=form1.Height-5;
   unit4.graf.show;
  end
  else begin
   ...
   label1.caption:='С помощью кнопки ENTER';
   wwodxm.Height:=0;
   wwodxm.Top:=0;
   wwody.Height:=0;
   wwody.Top:=0;
   ...
   combobox1.left:=1120;
   unit4.graf.Hide;
   ochisti.Caption:='Очистить поле ввода';
...
//Ограничения
 ...
 //Если не меняется количесвто строк то выход из процедуры

 if (wwodx.Lines.Count < 2) then begin
 ...
 begin
   for p:=1 to length(wwodx.Lines[i]) do
    if wwodx.Lines[i][p]='.' then tchk:=true;
 end;

 if tchk then begin
 memo1.Text:=wwodx.Text;
 wwodx.Clear;
 for i:=0 to memo1.Lines.Count-1 do
 begin
   helpstr:=memo1.Lines[i];
...
//Ограничение по количеству измерений
...
if miss.numb = 0 then exit;

//Округление cлучайной погрешности, sko, среднего

ext:=0; ex:=1;
while miss.numb >= 100 do begin miss.numb:=miss.numb/10; inc(ext) end;
while miss.numb < 10 do begin miss.numb:=miss.numb*10; dec(ext) end;
...
procedure TForm1.wwodxmChange(Sender: TObject);
var

 xmas,ymas:dat;
 i,nx,ny,p,minN:integer;
 helpstr:string;
 tchk:boolean;

begin

 if wwodxm.Lines.Count>=50 then
 for i:=wwodxm.lines.Count downto 50 do wwodxm.lines.Delete(i);

 //Удаление аппроксимирующей и масштабирующей линнии при любом изменении

 unit4.graf.approximationSeries.clear;
 unit4.graf.scaleSeries.Clear;

 unit4.graf.yravnenie.caption:='';
 unit4.graf.vidfunc.Caption:='Вид функции:';
 unit4.graf.koffa.caption:='';
 unit4.graf.koffb.caption:='';
 unit4.graf.skoa.caption:='';
 unit4.graf.StringGrid1.RowCount:=1;

 //Замена . на ,

 tchk:=false;
 for i:=0 to wwodxm.Lines.Count-1 do
 begin
   for p:=1 to length(wwodxm.Lines[i]) do
    if wwodxm.Lines[i][p]='.' then tchk:=true;
 end;

 if tchk then begin
 memo1.Text:=wwodxm.Text;
 wwodxm.Clear;
 for i:=0 to memo1.Lines.Count-1 do
 begin
   helpstr:=memo1.Lines[i];
 ...
 // добавление экспериментальных точек на график

 if (nx=0) or (ny=0) then unit4.graf.experimentalPoints.Clear;
 if (nx<>0) and (ny<>0) then begin
 unit4.graf.experimentalPoints.Clear;
 if nx<=ny then minn:=nx
           else minn:=ny;
 for i:=0 to minn-1 do
   unit4.graf.experimentalPoints.AddXY(xmas[i],ymas[i]);
 end;
end;

procedure TForm1.wwodxmKeyPress(Sender: TObject; var Key: char);
begin
 if wwodxm.Lines.Count>49 then if key=#13 then key:=chr(0);
 case key of
    '0'..'9',#8,#44,#13:;
    #46: key:= #44;
    else key:=chr(0);
...
begin
 ...
 tchk:=false;
 for i:=0 to wwody.Lines.Count-1 do
 begin
   for p:=1 to length(wwody.Lines[i]) do
    if wwody.Lines[i][p]='.' then tchk:=true;
 end;

 if tchk then begin
 memo1.Text:=wwody.Text;
 wwody.Clear;
 for i:=0 to memo1.Lines.Count-1 do
 begin
   helpstr:=memo1.Lines[i];
...
begin
 if wwody.Lines.Count>49 then if key=#13 then key:=chr(0);
 case key of
    '0'..'9',#8,#44,#13:;
    #46: key:= #44;
    else key:=chr(0);
